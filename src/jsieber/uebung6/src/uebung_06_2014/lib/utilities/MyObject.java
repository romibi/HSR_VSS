package uebung_06_2014.lib.utilities;

import java.io.Serializable;
import java.util.Date;
import java.util.Random;

import uebung_11.lib.Synchronization.ConditionVariable;
import uebung_11.lib.Synchronization.MessagePassing;
import uebung_11.lib.Synchronization.Semaphore;
...
public abstract class MyObject extends Object implements Serializable {
	...
	private static final long serialVersionUID = 6439053209351147400L;

	// Comment this out if you get tired of seeing it every time you run a
	// program.
	static {
		System.out.println("Java version=" + System.getProperty("java.version")
				+ ", Java vendor=" + System.getProperty("java.vendor")
				+ "\nOS name=" + System.getProperty("os.name") + ", OS arch="
				+ System.getProperty("os.arch") + ", OS version="
				+ System.getProperty("os.version") + "\n" + new Date());
	}
	/*
	 * Windows 95 time-slices threads but Solaris does not as of JDK 1.1. To
	 * enable time slicing on Solaris, set the boolean variable
	...
	private static Scheduler scheduler = timeSlicingEnsured
			&& OSname.equals("Solaris") ? new Scheduler(timeSlice) : null;
	...
	protected static final int nap(int napTimeMS) {
		/*
		 * Compute how much the actual sleep exceeded the request and return
		 * that to the user. If the sleep was interrupted, then the computation
		 * will be negative. Let the user decide what to do.
		 */
		long napStart = age();
		try {
	...
	public static void main(String[] args) throws Exception {
		System.out
				.println("Copyright (c) 1997 Stephen J. Hartley. All Rights Reserved.");
