...
public class Scheduler extends MyObject implements Runnable {
	...
	private static final long serialVersionUID = -8219516604128106099L;
	...
	public Scheduler(int timeSlice) {
		this(timeSlice, false, false);
	}

	public void run() {
		int napping;
		System.out.println(getName() + ": timeSlice=" + timeSlice
				+ " randomSlice=" + randomSlice + " priority="
				+ t.getPriority());
		while (true) {
			if (randomSlice)
				napping = 1 + (int) (2 * random(timeSlice));
			else
				napping = timeSlice;
			nap(napping);
