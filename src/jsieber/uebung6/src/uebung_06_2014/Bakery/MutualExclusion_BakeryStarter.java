...
import uebung_06_2014.lib.utilities.GetOpt;
import uebung_06_2014.lib.utilities.MyObject;


/*
 * Programm Argumente
 * -R15 -n4 1 1 1 1 8 3 8 3
 */

public class MutualExclusion_BakeryStarter extends MyObject {

	private static final long serialVersionUID = -4915901559352234624L;
	...
	public static void main(String[] args) {
		...
		Bakery arb = new Bakery(numNodes);

		// the Node threads are started in the constructor
		for (int i = 0; i < numNodes; i++)
			new Node("Node", i, napOutsideCS[i] * 1000, napInsideCS[i] * 1000,
					arb);
...
class Node extends MyObject implements Runnable {
	...
	private static final long serialVersionUID = 2765765243738931910L;
	private int id = 0;
	private int napOutsideCS = 0; // both are in
	private int napInsideCS = 0; // milliseconds
	private Bakery arb = null;

	public Node(String name, int id, int napOutsideCS, int napInsideCS,
			Bakery arb) {
		super(name + " " + id);
		this.id = id;
